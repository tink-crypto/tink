...
generate_keys() {
  ...
  $TINKEY_CLI create --key-template $key_template --outFormat BINARY \
      --out $priv_key_file  || exit 1
  $TINKEY_CLI create-public-keyset --inFormat BINARY --in $priv_key_file \
      --outFormat BINARY --out $pub_key_file  || exit 1
