...
TEST_F(KeysetManagerTest, testBasicOperations) {
  ...
  // Verify the keyset.
  auto keyset = TestUtil::GetKeyset(*(keyset_manager->GetKeysetHandle()));
  ...
  keyset = TestUtil::GetKeyset(*(keyset_manager->GetKeysetHandle()));
  ...
  auto keyset_2 = TestUtil::GetKeyset(*(keyset_manager_2->GetKeysetHandle()));
