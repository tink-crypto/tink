...
TEST_F(RsaSsaPssSignKeyManagerTest, PublicKeyExtraction) {
  RsaSsaPssSignKeyManager sign_key_type_manager;
  RsaSsaPssVerifyKeyManager verify_key_type_manager;
  ...
  EXPECT_EQ(RsaSsaPssVerifyKeyManager().get_key_type(),
...
TEST_F(RsaSsaPssSignKeyManagerTest, NewKeyErrors) {
  ...
  const KeyFactory& key_factory = sign_key_manager->get_key_factory();
