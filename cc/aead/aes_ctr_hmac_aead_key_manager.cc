...
AesCtrHmacAeadKeyManager::GetPrimitive(const KeyData& key_data) const {
...
StatusOr<std::unique_ptr<Aead>>
AesCtrHmacAeadKeyManager::GetPrimitive(const Message& key) const {
  std::string key_type =
...
StatusOr<std::unique_ptr<Aead>>
AesCtrHmacAeadKeyManager::GetPrimitiveImpl(
  ...
  auto hmac_result = Registry::GetPrimitive<Mac>(
      kHmacKeyType,
      aes_ctr_hmac_aead_key.hmac_key());
...
Status AesCtrHmacAeadKeyManager::Validate(const AesCtrHmacAeadKey& key) const {
  ...
  if (aes_key_size != 16 && aes_key_size != 24 && aes_key_size != 32) {
      return ToStatusF(
          ...
          "supported sizes: 16, 24, or 32 bytes.", aes_key_size);
  }
