...
AesGcmBoringSsl::AesGcmBoringSsl(google::protobuf::StringPiece key_value,
                                 const EVP_CIPHER* cipher)
    : key_(key_value), cipher_(cipher) {}

crypto::tink::util::StatusOr<std::unique_ptr<Aead>> AesGcmBoringSsl::New(
    google::protobuf::StringPiece key_value) {
  const EVP_CIPHER* cipher = GetCipherForKeySize(key_value.size());
  if (cipher == nullptr) {
    return crypto::tink::util::Status(crypto::tink::util::error::INTERNAL, "invalid key size");
  }
  std::unique_ptr<Aead> aead(new AesGcmBoringSsl(key_value, cipher));
  return std::move(aead);
}

crypto::tink::util::StatusOr<std::string> AesGcmBoringSsl::Encrypt(
  ...
  if (ret != 1) {
    return crypto::tink::util::Status(crypto::tink::util::error::INTERNAL, "EVP_EncryptInit_ex failed");
  }
  ret = EVP_CIPHER_CTX_ctrl(ctx.get(), EVP_CTRL_GCM_SET_IVLEN, IV_SIZE_IN_BYTES,
                            nullptr);
  if (ret != 1) {
    return crypto::tink::util::Status(crypto::tink::util::error::INTERNAL, "IV length not supported");
  }
  ...
  if (ret != 1) {
    return crypto::tink::util::Status(crypto::tink::util::error::INTERNAL, "Could not initialize ctx");
  }
  int len;
  ...
  if (ret != 1) {
    return crypto::tink::util::Status(crypto::tink::util::error::INTERNAL, "AAD is not supported");
  }
  ...
  ret = EVP_EncryptUpdate(ctx.get(), &ct[written], &len,
                          ...
                          plaintext.size());
  if (ret != 1) {
    crypto::tink::util::Status(crypto::tink::util::error::INTERNAL, "Encryption failed");
  }
  written += len;
  ret = EVP_EncryptFinal_ex(ctx.get(), &ct[written], &len);
  written += len;
  if (ret != 1) {
    crypto::tink::util::Status(crypto::tink::util::error::INTERNAL, "EVP_EncryptFinal_ex failed");
  }
  ret = EVP_CIPHER_CTX_ctrl(ctx.get(), EVP_CTRL_GCM_GET_TAG, TAG_SIZE_IN_BYTES,
                            &ct[written]);
  if (ret != 1) {
    crypto::tink::util::Status(crypto::tink::util::error::INTERNAL, "Could not compute tag");
  }
  written += TAG_SIZE_IN_BYTES;
  if (written != ciphertext_size) {
    crypto::tink::util::Status(crypto::tink::util::error::INTERNAL, "Incorrect ciphertext size");
  }
  return std::string(reinterpret_cast<const char*>(&ct[0]), written);
}

crypto::tink::util::StatusOr<std::string> AesGcmBoringSsl::Decrypt(
    google::protobuf::StringPiece ciphertext,
    google::protobuf::StringPiece additional_data) const {
  if (ciphertext.size() < IV_SIZE_IN_BYTES + TAG_SIZE_IN_BYTES) {
    return crypto::tink::util::Status(crypto::tink::util::error::INTERNAL, "Ciphertext too short");
  }
  ...
  if (ret != 1) {
    return crypto::tink::util::Status(crypto::tink::util::error::INTERNAL, "EVP_DecryptInit_ex failed");
  }
  ...
  if (ret != 1) {
    return crypto::tink::util::Status(crypto::tink::util::error::INTERNAL, "Could not initialize key");
  }
  ...
  ret = EVP_DecryptUpdate(
      ...
      plaintext_size);
  if (ret != 1) {
    crypto::tink::util::Status(crypto::tink::util::error::INTERNAL, "Decryption failed");
  }
  written += len;
  ...
  ret = EVP_CIPHER_CTX_ctrl(ctx.get(), EVP_CTRL_GCM_SET_TAG, TAG_SIZE_IN_BYTES,
                            tag);
  if (ret != 1) {
    return crypto::tink::util::Status(crypto::tink::util::error::INTERNAL, "Could not set tag");
  }

  ret = EVP_DecryptFinal_ex(ctx.get(), &pt[written], &len);
  written += len;
  if (ret != 1) {
    return crypto::tink::util::Status(crypto::tink::util::error::INTERNAL, "Authentication failed");
  }
  if (written != plaintext_size) {
    return crypto::tink::util::Status(crypto::tink::util::error::INTERNAL, "Incorrect plaintext size");
  }
