...
Status Validate(absl::string_view ikm,
  ...
  if (ikm.size() < 16 || ikm.size() < derived_key_size_in_bytes) {
    return Status(util::error::INVALID_ARGUMENT, "ikm too small");
  }
  if (derived_key_size_in_bytes != 16 && derived_key_size_in_bytes != 32) {
    return util::Status(util::error::INVALID_ARGUMENT,
                        "derived_key_size_in_bytes must be 16 or 32");
  }
  ...
  if (ciphertext_segment_size <
      first_segment_offset + derived_key_size_in_bytes) {
    return util::Status(util::error::INVALID_ARGUMENT,
...
AesGcmHkdfStreaming::New(absl::string_view ikm,
                         ...
                         int ciphertext_segment_size,
                         int first_segment_offset) {
  auto status = Validate(ikm, hkdf_hash, derived_key_size_in_bytes,
  ...
  streaming_aead->derived_key_size_in_bytes_ = derived_key_size_in_bytes;
...
AesGcmHkdfStreaming::NewSegmentEncrypter(
  ...
  params.salt = Random::GetRandomBytes(derived_key_size_in_bytes_);
  auto hkdf_result = Hkdf::ComputeHkdf(
      hkdf_hash_, ikm_, params.salt, associated_data,
      derived_key_size_in_bytes_);
