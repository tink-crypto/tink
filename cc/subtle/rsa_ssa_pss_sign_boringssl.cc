...
util::Status CopyKey(const SubtleUtilBoringSSL::RsaPrivateKey& key, RSA* rsa) {
...
util::Status CopyPrimeFactors(const SubtleUtilBoringSSL::RsaPrivateKey& key,
                              RSA* rsa) {
...
util::Status CopyCrtParams(const SubtleUtilBoringSSL::RsaPrivateKey& key,
                           RSA* rsa) {
...
util::StatusOr<std::unique_ptr<PublicKeySign>> RsaSsaPssSignBoringSsl::New(
  ...
  {
    auto st = CopyKey(private_key, rsa.get());
    if (!st.ok()) return st;
  }
  {
    auto st = CopyPrimeFactors(private_key, rsa.get());
    if (!st.ok()) return st;
  }
  {
    auto st = CopyCrtParams(private_key, rsa.get());
    if (!st.ok()) return st;
  }
