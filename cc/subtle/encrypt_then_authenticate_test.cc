...
// We use CTR but the RFC uses CBC mode, so it's not possible to compare plaintexts.
// However, the tests are still valueable to ensure that we correcly compute HMAC
// over ciphertext and aad.
struct TestVector {
  std::string mac_key;
  std::string enc_key;
...
static util::StatusOr<std::unique_ptr<Aead>> createAead2(
  ...
  auto cipher_res = EncryptThenAuthenticate::New(
      std::move(ind_cipher_res.ValueOrDie()),
      std::move(mac_res.ValueOrDie()),
      tag_size);
  if (!cipher_res.ok()) {
...
TEST(AesGcmBoringSslTest, testRfcVectors) {
  for (const TestVector& test : test_vectors) {
    ...
    auto res = createAead2(
        enc_key, test.iv_size, mac_key, test.tag_size, test.hash_type);
...
TEST(EncryptThenAuthenticateTest, testEncryptDecrypt) {
  ...
  int tag_size = 16;
  auto res = createAead(
      encryption_key_size, iv_size, mac_key_size, tag_size, HashType::SHA1);
  EXPECT_TRUE(res.ok()) << res.status();
  auto cipher = std::move(res.ValueOrDie());
