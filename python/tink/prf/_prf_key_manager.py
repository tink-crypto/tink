...
class PrfSetCcToPyWrapper(_prf_set.PrfSet):
  """Transforms C++ PrfSet primitive into a Python PrfSet primitive."""

  def __init__(self, cc_primitive: tink_bindings.PrfSet):
    # cc_primitive is a PrfSet that only contains one PRF.
    self._prf = PrfCcToPyWrapper(cc_primitive)
...
def register() -> None:
  ...
  for ident in (
    ...
    key_manager = core.KeyManagerCcToPyWrapper(
        ...
        _prf_set.PrfSet, PrfSetCcToPyWrapper)
