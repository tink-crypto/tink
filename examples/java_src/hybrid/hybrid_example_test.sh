...
##### Run encryption
test_command ${HYBRID_CLI} encrypt ${PUBLIC_KEYSET_FILE} ${INPUT_FILE} ${INPUT_FILE}.ciphertext
...
##### Run decryption
test_command ${HYBRID_CLI} decrypt ${PRIVATE_KEYSET_FILE} ${INPUT_FILE}.ciphertext ${INPUT_FILE}.plaintext
if [[ ${TEST_STATUS} -eq 0 ]]; then
...
test_command ${HYBRID_CLI} encrypt ${PUBLIC_KEYSET_FILE} ${INPUT_FILE} ${INPUT_FILE}.ciphertext "${CONTEXT_INFORMATION}"
if [[ ${TEST_STATUS} -eq 0 ]]; then
  echo "+++ Encryption successful."
else
  echo "--- Encryption failed."
...
##### Run decryption
test_command ${HYBRID_CLI} decrypt ${PRIVATE_KEYSET_FILE} ${INPUT_FILE}.ciphertext ${INPUT_FILE}.plaintext "${CONTEXT_INFORMATION}"
if [[ ${TEST_STATUS} -eq 0 ]]; then
  echo "+++ Decryption successful."
else
  echo "--- Decryption failed."
...
##### Run encryption
test_command ${HYBRID_CLI} encrypt ${PRIVATE_KEYSET_FILE} ${INPUT_FILE} ${INPUT_FILE}.ciphertext
if [[ ${TEST_STATUS} -eq 1 ]]; then
