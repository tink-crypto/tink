...
describe('hmac test', () => {
  ...
  it('constructor', async () => {
    ...
    try {
      await hmacFromRawKey(
          'SHA-384', random.randBytes(15), 16);  // invalid key size
      // TODO(b/115974209): This case does not throw an exception.
      // Preserving old behavior when moving to
      // https://www.typescriptlang.org/tsconfig#useUnknownInCatchVariables
      // tslint:disable-next-line:no-any
    ...
    try {
      await hmacFromRawKey(
          'SHA-384', random.randBytes(16), 9);  // tag size too short
      fail('Should throw an exception.');
      // Preserving old behavior when moving to
      // https://www.typescriptlang.org/tsconfig#useUnknownInCatchVariables
      // tslint:disable-next-line:no-any
    ...
    try {
      await hmacFromRawKey(
          'SHA-384', random.randBytes(16), 49);  // tag size too long
      fail('Should throw an exception.');
      // Preserving old behavior when moving to
      // https://www.typescriptlang.org/tsconfig#useUnknownInCatchVariables
      // tslint:disable-next-line:no-any
      ...
      expect(e.toString())
          .toBe(
              'InvalidArgumentsException: tag too long, must not be larger than 48 bytes');
    }
  ...
  it('with test vectors', async () => {
    ...
    const NIST_TEST_VECTORS: TestVector[] = [
      ...
      {
        algo: 'SHA-256',
        key: '6f35628d65813435534b5d67fbdb54cb33403d04e843103e6399f806cb5df95' +
            'febbdd61236f33245',
        message:
      ...
      },
      {
        algo: 'SHA-384',
        key:
            'f16ad73790ca39c7f9856c4483202e7f8e0c8283c7d50d6da79cc07d3dc7b76c' +
            '2ef76100fa3ae2df8083b5a1c5579628f1c8',
        message:
            '9870007654ebc3d28f883bb832e0b31700f923d9c9b10168e0605971cfb920e8' +
            '48f1c64c5f240a2cf7f412ea7a73bbbfce432eff84fbb49e52cdcbf4c36679bd' +
            '2d16e064e4311381adb528a0752c8e4443d4a12b6cfe7cd406b40e3f9e9e71f4' +
            '2e27764649db85d99913a4628bd5d5ae49f6a5e6e9810211e35d4ddac929b093',
        tag: '79e24a203bf42074e72c8b4a0222aface3e8ce7b4004cec2'
      },
      {
        algo: 'SHA-512',
        key:
