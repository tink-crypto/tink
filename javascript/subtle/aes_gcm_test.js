...
if ((Environment.IS_WEBCRYPTO_AVAILABLE)) {
  testSuite({
    async testBasic() {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        assertEquals(
            12 /* iv */ + msg.length + 16 /* tag */, ciphertext.length);
  ...
  testSuite({
    // We cannot run any tests because WebCrypto is unavailable, so we use a
    // dummy test to satisfy js_test_suite's requirement that the test suite
    // must be non empty.
    testDummy() {
      assertEquals(1, 1);
    }
