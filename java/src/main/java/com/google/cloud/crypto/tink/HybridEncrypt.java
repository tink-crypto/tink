 ...
 * ciphertext the same value must be provided for decryption operation (cf. {@link HybridDecrypt}).
 * <p>
...
public interface HybridEncrypt {
   ...
   */
  ListenableFuture<byte[]> asyncEncrypt(final byte[] plaintext, final byte[] contextInfo);
}
