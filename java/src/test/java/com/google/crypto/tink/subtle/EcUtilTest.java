...
/** Unit tests for {@link com.google.crypto.tink.subtle.EcUtil}. */
...
public class EcUtilTest {
  ...
  static class TestVector1 {
    EcUtil.CurveTypeEnum curve;
    public String pubX;
    public String pubY;

    public TestVector1(
        EcUtil.CurveTypeEnum curve, String pubX, String pubY) {
      this.curve = curve;
      this.pubX = pubX;
      this.pubY = pubY;
    }
    ...
    public EllipticCurve getCurve() throws NoSuchAlgorithmException {
      return EcUtil.getCurveSpec(curve).getCurve();
    }
  }

  public static final TestVector1[] testVectors1 = new TestVector1[] {
    new TestVector1(EcUtil.CurveTypeEnum.NIST_P256,
        ...
        new TestVector1(EcUtil.CurveTypeEnum.NIST_P384,
        ...
        new TestVector1(EcUtil.CurveTypeEnum.NIST_P521,
  ...
  public void testPointOnCurve() throws Exception {
    for (int i = 0; i < testVectors1.length; i++) {
      ...
      try {
        EcUtil.checkPointOnCurve(pubPoint, testVectors1[i].getCurve());
  ...
  public void testPointNotOnCurve() throws Exception {
    for (int i = 0; i < testVectors1.length; i++) {
      ECPoint pubPoint = new ECPoint(new BigInteger(testVectors1[i].pubX, 16),
  ...
  protected static class TestVector2 {
    protected EcUtil.CurveTypeEnum curve;
    protected EcUtil.PointFormatEnum format;
  ...
  protected static TestVector2[] testVectors2 = {
    // NIST_P256
    new TestVector2(
        EcUtil.CurveTypeEnum.NIST_P256,
    ...
    new TestVector2(
        EcUtil.CurveTypeEnum.NIST_P256,
        EcUtil.PointFormatEnum.COMPRESSED,
    ...
    // NIST_P384
    new TestVector2(
        EcUtil.CurveTypeEnum.NIST_P384,
        EcUtil.PointFormatEnum.UNCOMPRESSED,
    ...
    // NIST_P521
    new TestVector2(
        EcUtil.CurveTypeEnum.NIST_P521,
        EcUtil.PointFormatEnum.UNCOMPRESSED,
  ...
  public void testPointDecode() throws Exception {
    for (TestVector2 test : testVectors2) {
      EllipticCurve curve = EcUtil.getCurveSpec(test.curve).getCurve();
      ECPoint p = EcUtil.ecPointDecode(curve, test.format, test.encoded);
  ...
  public void testPointEncode() throws Exception {
    for (TestVector2 test : testVectors2) {
      ...
      byte[] encoded = EcUtil.ecPointEncode(curve, test.format, p);
