...
public class EllipticCurvesTest {
  ...
  // These tests and test vectors are copied from Wycheproof.
  // TODO(b/66825199): pulling these test vectors from Wycheproof once Wycheproof has
  // exported them.
  ...
  public static class EcPublicKeyTestVector {
    ...
    public X509EncodedKeySpec getX509EncodedKeySpec() {
      return new X509EncodedKeySpec(Hex.decode(encoded));
    }
  }
  ...
  public void testComputeSharedSecret_invalidCurve_shouldFail() throws Exception {
    ...
    try {
      EllipticCurves.computeSharedSecret(
          (ECPrivateKey) p256.getPrivate(), (ECPublicKey) p384.getPublic());
      fail("computeSharedSecret with keys on different curves should fail");
    } catch (GeneralSecurityException ex) {
      // OK because the curves are different.
      System.out.println("testComputeSharedSecret_invalidCurve_shouldFail throws " + ex.toString());
    }

    try {
      EllipticCurves.computeSharedSecret(
          (ECPrivateKey) p384.getPrivate(), (ECPublicKey) p256.getPublic());
      fail("computeSharedSecret with keys on different curves should fail");
    } catch (GeneralSecurityException ex) {
      // OK because the curves are different.
      System.out.println("testComputeSharedSecret_invalidCurve_shouldFail throws " + ex.toString());
