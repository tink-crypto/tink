...
public class JsonKeysetReaderTest {
  ...
  public void testRead_staticMethods_validKeyset_shouldWork() throws Exception {
    ...
    KeysetHandle handle4 =
        CleartextKeysetHandle.read(JsonKeysetReader.withJsonObject(
            JsonParser.parseString(JSON_KEYSET).getAsJsonObject()));
  ...
  public void testReadEncrypted_missingEncryptedKeyset_shouldThrowException() throws Exception {
    ...
    JsonObject json = JsonParser.parseString(
        new String(outputStream.toByteArray(), UTF_8)).getAsJsonObject();
  ...
  public void testReadKeyset_hugeKeyId_throws() throws Exception {
    JsonObject json = JsonParser.parseString(createJsonKeysetWithId("4294967275")).getAsJsonObject(); // 2^32 - 21
    Keyset keyset = JsonKeysetReader.withJsonObject(json).read();
    assertThat(keyset.getPrimaryKeyId()).isEqualTo(-21);
  }
}
