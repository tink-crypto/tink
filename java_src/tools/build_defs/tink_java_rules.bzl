...
def collect_android_libraries_and_make_test_suite(name, shard_count = 1):
    """Creates an android test suite for previous tink_android_test.

    Creates, for a bunch of devices, an android_instrumentation_test which
    runs all tests previously defined in tink_android_test, but only for
    the versions specified there.

    If the tink_android_test target has a tag "android_min_version:xx", the
