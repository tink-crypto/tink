...
- (void)testConfigContents {
  std::string key_type = "type.googleapis.com/google.crypto.tink.EciesAeadHkdfPublicKey";
  ...
  XCTAssertTrue("TinkHybridEncrypt" == config.entry(3).catalogue_name());
  XCTAssertTrue("HybridEncrypt" == config.entry(3).primitive_name());
  XCTAssertTrue(key_type == config.entry(3).type_url());
