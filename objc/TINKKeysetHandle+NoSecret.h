...
/**
 * Category that creates keyset handles from keysets with no secret key material.
 */
@interface TINKKeysetHandle (NoSecret)

/**
 * Creates a TINKKeysetHandle from a keyset obtained via @c reader or fails if there is secret key
 * material in the keyset.
 *
 * @param reader The reader that will read the keyset.
